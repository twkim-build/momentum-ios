...
struct HabitListViewModelTests {
    ...
    @Test
    func save_success_updatesHabit async {
        let habitID = UUID()
        let habit = HabitTestFactory.makeHabitDetailItem(id: habitID)
        ...
        #expect(didSave == true)
        #expect(repository.updateHabitCallCount == 1)
        #expect(repository.lastUpdatedHabitID == habitID)
        #expect(repository.lastUpdatedName == "Updated")
    }
